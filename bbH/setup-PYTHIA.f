 1    subroutine setup_PYTHIA_tune
...
C   
c 340 AMBT1 tune
      tune=powheginput("#pythiatune")
      if(tune.lt.0) tune=320
      call PYTUNE(tune)
...
C---Higgs decay; 
      ...
      if ((hdecaymode.lt.-1).or.(hdecaymode.gt.12)) then
         write(*,*) "UPINIT: Higgs decay mode not allowed"
         stop
      endif   
...
c for comparison: remove bratio from xsec:
      xwgtup = xwgtup/bratio
