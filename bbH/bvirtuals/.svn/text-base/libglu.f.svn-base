...
c---------------------------------------------------------------------
     ...
     $     mirpole_1,mirpole_2)
      ...
      real*8 pt(4),ptb(4),ph(4),p3(4),p4(4)
      real*8 s3,beta,lbeta,lmu,lt,p1p2,p1p3,p1p4,p2p3,p2p4
...
c
      call gluform_s(s,t1,t2,u1,u2,mt,mh,
     $     kmvs1,kmvs2,kmvs3,kmvs4,kmvs5)
c
c t-channel: 
...
c
      call gluform_t(s,t1,t2,u1,u2,mt,mh,
     $     kmvt1,kmvt2,kmvt3,kmvt4,kmvt5)
c
...
c
      call gluform_t(s,u1,u2,t1,t2,mt,mh,
     $     kmvu1,kmvu2,kmvu3,kmvu4,kmvu5)
c
c Pentagons (P2,P3,P4):
c
...
c Born      
      ...
      p2p3=(mt**2-u1)/2d0
      p2p4=(mt**2-t1)/2d0
      beta=dsqrt(1d0-2d0*mt**2/(p1p2+mt**2))
      lbeta=dlog((1d0+beta)/(1d0-beta))
      lmu=dlog(4d0*pi*muedr**2/mt**2)
c color factors
...
c coefficient of single IR pole:
     ...
     $     2d0*p1p2/(p1p2+mt**2)/beta*lbeta*a0_abnab
      ...
      do i=1,3
         ceps1(i)=ceps2(i)*(lmu-gammaE)+ceps1s(i)
      enddo
c poles
      mirpole_1=0d0
      mirpole_2=0d0
      ...
      do i=1,3
         ...
         dirfinite=dirfinite+cir(i)*(ceps1s(i)*(lmu-GammaE)+
     $     ceps2(i)*(lmu**2/2d0-gammaE*lmu+gammaE**2/2d0+pi**2/12d0))
      enddo
c nlf is the number of light flavors
      mirpole_1=mirpole_1+2d0*(2d0/3d0*nlf-8d0/3d0*ncf+1d0/ncf)*sig0gg
c      matv_gg=matv_gg+2d0*(dirfinite+
c     $     (2d0/3d0*nlf-8d0/3d0*ncf+1d0/ncf)*sig0gg*(lmu-GammaE))
c POWHEG convention:
      lt=dlog(muedr**2/mt**2)
      mirpole_1=0d0
      dirfinite=0d0
      do i=1,3
         mirpole_1=mirpole_1+2d0*cir(i)*(ceps1s(i)+ceps2(i)*lt)
         dirfinite=dirfinite+cir(i)*(ceps1s(i)*lt+
     $     ceps2(i)*(lt**2+pi**2/3d0)/2d0)
      enddo
c nlf is the number of light flavors
      mirpole_1=mirpole_1+2d0*(2d0/3d0*nlf-8d0/3d0*ncf+1d0/ncf)*sig0gg
      matv_gg=matv_gg+2d0*(dirfinite+
     $     (2d0/3d0*nlf-8d0/3d0*ncf+1d0/ncf)*sig0gg*lt)
      return
      end
c---------------------------------------------------------------------
*
...
c i sigma_mu_nu=i (q_mu q_nu - q^2 g_mu_nu) sigmagg 
      call ggself(s,mt,mtop,mh,sigmagg)
...
c -g_s f_abc Lambda_mu_nu_ro:
      call ggvert(s,mt,mtop,mh,vertexgg)
...
*
c---------------------------------------------------------------------
      subroutine ggself(s,mb,mt,mh,sigma)
      implicit none
      real*8 ncf,cg,cf,mb,mt,mh,s
...
c nlf is the number of light flavors:
     ...
     $     cg*(-5d0/3d0*dlog(s/muedr**2)+31d0/9d0)
      if(nlf.eq.4)then
         call bfunc(s,mb,mb,b0s,b1s,b20)
         sigma=sigma+
     $        2d0/3d0*(2d0*mb**2/s*dlog(mb**2/muedr**2)+
     $        (1d0+2d0*mb**2/s)*b0s-1d0/3d0)
      endif
c add counterterm:
      sigma=sigma+dZ3
...
*
c---------------------------------------------------------------------
      subroutine ggvert(s,mf,mt,mh,vertex)
      implicit none
      real*8 ncf,cg,cf,mf,mt,mh,s
...
c b00 is also UV divergent:
      b00=dcmplx(-dlog(mf**2/muedr**2),0d0)
      c00=1d0/s*dcmplx(dlog(mf**2/s)**2/2d0-2d0/3d0*pi**2,0d0)
...
c nlf is the number of light flavors:
     ...
     $     9d0*b00)
      if(nlf.eq.4)then
         call ccfunc(0d0,s,-s/2d0,mf,mf,mf,
     $        c0,c11,c12,c21,c22,c20,c23)
         call bfunc(s,mf,mf,b0s,b1s,b20)
         vertex=vertex+(5d0/3d0*b0s-4d0*c20+4d0/9d0-4d0*mf**2*c12+
     $     4d0/3d0*mf**2/s*(b0s+dlog(mf**2/muedr**2)))
      endif
...
*
c---------------------------------------------------------------------
      subroutine renorm(mf,mt,mh)
      implicit none
      real*8 mf,mt,mh,dZ3,dZ2,dZm,dZ1F,dZg
...
c -G[1+eps] (4 pi mu^2/mf^2)^eps 1/2/eps_IR has been subtracted
      db0=-1d0/mf**2
c dZ3 in modified MSbar:
c UV divergences have been subtracted: N_eps=2/eps-gamma_E+ln(4pi)
      dZ3=2d0/3d0*dlog(mt**2/muedr**2)
      if(nlf.eq.4)then
       dZ3=dZ3+2d0/3d0*dlog(mf**2/muedr**2)
      endif
      dZ2=cf*(dlog(mf**2/muedr**2)+4d0*mf**2*db0)
      dZm=-cf*(-3d0*dlog(mf**2/muedr**2)+4d0)
      dZ1F=dZ2
      dZg=-1d0/3d0*dlog(mt**2/muedr**2)
      if(nlf.eq.4)then
       dZg=dZg-1d0/3d0*dlog(mf**2/muedr**2)
      endif
      return
      end
c---------------------------------------------------------------------
...
c V7(1)
      ...
      kmvf1(1,17)=2d0*(fvf1(1)-2d0*mt*fvf1(2)-
     $     2d0*(mt**2-p1p3)*fvf1(6))
...
c V8(3)
     ...
     $     +(p3p4-p2p3)*fvf1(5))
      ...
      kmvf2(2,25)=2d0*(fvf1(1)+2d0*mt*fvf1(2)+
     $     2d0*(p2p4-mt**2)*fvf1(6))
...
*
      ...
      c0=C0_(xpi1,xpi2,q12,m0,mi1,mi2)
c      write(6,*)c0
...
c      endif
      ...
      if(dabs(xpi1).ne.0d0.and.dabs(xpi1).lt.1d-6)xpi1=0d0
      if(dabs(xpi2).ne.0d0.and.dabs(xpi2).lt.1d-6)xpi2=0d0
      c0=C0_(xpi1,q12,xpi2,m0,mi1,mi2)
c
...
*********************************************************************
      ...
      d0(0)=D0_(p2,p3,p4,p15,p23,p34,m1,m2,m3,m4)
      if (np.eq.1) then
         d0(1)=D0_ir(xmt,p5,p12,p3,p4,p34,p45,m4,m2,m3)
      else
         d0(1)=D0_(p12,p3,p4,p5,p45,p34,m0,m2,m3,m4)
      endif
      ...
      elseif(np.eq.2.or.np.eq.4)then
         d0(4)=D0_(p1,p2,p3,p45,p12,p23,m0,m1,m2,m3)
      elseif(np.eq.3)then
         d0(4)=D0_(p1,p4,p3,p45,p12,p23,m0,m1,m2,m3)
      endif
      y(0,0) = 2d0*m02
      y(1,1) = 2d0*m12
      y(2,2) = 2d0*m22
...
c new version:
      goto 100
c 100  d0new = dreal(D0_(q1,q2,q3,q6,q4,q7,M1,M2,M3,M4))
 100  d0new = dreal(D0_(q1,q3,q7,q4,q6,q2,M1,M2,M3,M4))
      d1(0) = d0new
      s11 = (c1134(0)-c1234(0)-cf1*d1(0))/2d0
      s12 = (c1124(0)-c1234(0)-cf2*d1(0))/2d0
      s13 = (c1123(0)-c1234(0)-cf3*d1(0))/2d0
